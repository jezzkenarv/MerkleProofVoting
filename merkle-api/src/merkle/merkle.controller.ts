...
export class MerkleController {
    ...
    @Get('ballot/:id/results')
    async getBallotResults(@Param('id') id: string) {
        try {
            return {
                success: true,
                data: await this.merkleService.getBallotInfo(Number(id))
            };
        } catch (error) {
            console.error('❌ Error in getBallotResults:', error);
            throw new HttpException(
                {
                    success: false,
                    message: error.message || `Failed to fetch ballot results for ${id}`,
                },
    ...
    getContractAddress() {
        return {
            success: true,
            data: this.merkleService.getContractAddress()
        };
    }
    ...
    getServerWalletAddress() {
        return {
            success: true,
            data: this.merkleService.getServerWalletAddress()
        };
    }
